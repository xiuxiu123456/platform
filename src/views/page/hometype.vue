      ...
      <div class="dt-m">
        <div class="rjImg"  @click="showCheckItem">
          ...
          <div class="select-b" v-show="inshowItem">
            <CheckboxGroup  v-model="disabledGroup" @on-change="checkedHomeType">
              <Checkbox :label="item.dictValue" v-for="item in typeNameList" :key="item.dictValue">{{item.dictItem}}</Checkbox>
            </CheckboxGroup >
          </div>
      </div>
...
export default {
  ...
  methods:{
    ...
    loadDataEcharts () {
      ...
      this.$axios.post('/map/queryTypeInfoStatic', this.param)
      .then((resp) => {
        if(resp.data.code === 1){
    ...
    loadDataMap () {
      ...
      if (this.checkeArray.length > 0) {
        loadParam.typeArray = this.checkeArray;
      }
    ...
    initEcharts () {
      ...
      this.dataList.forEach(data => {
        if (data.countNum && data.typeName) {
          this.num_series_data.legend_data.push(getDictByItem('住房类型', data.typeName).dictItem);
          let obj = {};
          obj.name = getDictByItem('住房类型', data.typeName).dictItem;
