    ...
    <div class="amap-wrapper"  id="container" >
      ...
      <div class="dt-box">
        ...
        <div class="content-dt">
          <div class="left-dt">
            <div><em class="pg1"></em>前期</div>
            <div><em class="pg3"></em>基本建成</div>
            <div><em class="pg5"></em>供应</div>
          </div>
          <div class="left-dt">
            <div><em class="pg2"></em>开工</div>
            <div><em class="pg4"></em>竣工</div>
          </div>
        </div>
      </div>
      <div class="dt-m">
        <div class="rjImg"  @click="showCheckItem">
          ...
          <div class="select-b" v-show="inshowItem">
            <CheckboxGroup  v-model="disabledGroup" @on-change="checkedProgress">
              <Checkbox :label="item.dictValue" v-for="item in progressStatusList" :key="item.dictValue">{{item.dictItem}}</Checkbox>
            </CheckboxGroup >
          </div>
      </div>
...
export default {
  ...
  methods:{
    ...
    loadDataEcharts () {
      ...
      this.$axios.post('/map/queryStaticByStatus', this.param)
      .then((resp) => {
        if(resp.data.code === 1){
    ...
    loadDataMap () {
      ...
      if (this.checkeArray.length > 0) {
        loadParam.progressArray = this.checkeArray;
      }
    ...
    showProject () {
      ...
      this.projectInfos.forEach(pointData => {
        ...
        var marker = new AMap.Marker({
          ...
          content: '<div style="display: flex;"><div class="point-content point-content-' + pointData.progressStatus +'"></div>'+
    ...
    initEcharts () {
      ...
      this.dataList.forEach(data => {
        if (data.countNum && data.progressStatus) {
          this.num_series_data.legend_data.push(getDictByItem('进度状态', data.progressStatus).dictItem);
          let obj = {};
          obj.name = getDictByItem('进度状态', data.progressStatus).dictItem;
          obj.value = data.countNum;
          this.num_series_data.series_data.push(obj);
        }
        if (data.areaNum && data.progressStatus) {
          this.area_series_data.legend_data.push(getDictByItem('进度状态', data.progressStatus).dictItem);
          let obj = {};
          obj.name = getDictByItem('进度状态', data.progressStatus).dictItem;
          obj.value = data.areaNum;
          this.area_series_data.series_data.push(obj);
        }
        if (data.realBuildNum && data.progressStatus) {
          this.build_series_data.legend_data.push(getDictByItem('进度状态', data.progressStatus).dictItem);
          let obj = {};
          obj.name = getDictByItem('进度状态', data.progressStatus).dictItem;
