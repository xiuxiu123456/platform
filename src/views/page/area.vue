  ...
  <div class="area-container">
    <!-- <div  class="area-left" :class="{active:btnPostion}">
      ...
      <div class="left-echarts" v-show="!btnPostion">
        <div class="area-top">
          <div class="data">
            <Date-picker type="year" v-model="queryObj.startDate" placeholder="选择年" style="width: 192px" @on-change="initData"></Date-picker>
            <Date-picker type="year" v-model="queryObj.endDate" placeholder="选择年" style="width: 192px" @on-change="initData"></Date-picker>
          </div>
        </div>
...
export default {
  ...
  methods:{
    ...
    initData () {
      ...
      this.$axios.post('/map/queryAreaStatic', this.param)
      .then((resp) => {
        if(resp.data.code === 1){
    ...
    initEcharts () {
      ...
      this.dataList.forEach(data => {
        if (data.countNum) {
          this.num_series_data.legend_data.push(data.area_name);
          let obj = {};
          obj.name = data.area_name;
          obj.value = data.countNum;
          this.num_series_data.series_data.push(obj);
        }
        if (data.areaNum) {
          this.area_series_data.legend_data.push(data.area_name);
          let obj = {};
          obj.name = data.area_name;
        ...
        if (data.realBuildNum) {
          this.build_series_data.legend_data.push(data.area_name);
          let obj = {};
          obj.name = data.area_name;
    ...
    // initStree() {
    //   this.$axios.post('/wx/queryPolicy')
    //   .then((resp) => {
    //     if(resp.data.code === 1){
    //       console.log(resp)
    //     }
