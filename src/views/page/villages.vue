...
export default {
  ...
  methods: {
    initData () {
      this.$axios.post('/village/queryMapCount', this.param)
      .then((resp) => {
        if(resp.data.code === 1){
    ...
    clickMarker (markerData) {
      if ( markerData.lng && markerData.lat ) {
        ...
        this.$axios.post('village/queryVillageListByAreaId/'+markerData.areaId)
        .then((resp) => {
          if(resp.data.code === 1){
      ...
      this.villageInfoList.forEach(pointData => {
        var marker = new AMap.Marker({
          ...
          content: '<div style="display: flex;"><div class="point-content point-content-F"><div class="point-inset"></div></div>'+
                   '<div style="width:300px">' + pointData.villageName + '</div></div>'
        });
        let that = this;
        marker.on('click', function(){
    ...
    clickPoint (pointData) {
      ...
      var content = [
          '<div class="title-info-map">' + pointData.villageName + '</div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '占地面积：' + '</div><div class="info-text-area">' + (pointData.villageArea?pointData.villageArea:'') + 'm<sup>2</sup></div></div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '总建筑面积：' + '</div><div class="info-text-area">' + (pointData.villageBuild?pointData.villageBuild:'') + 'm<sup>2</sup></div></div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '总栋数：' + '</div><div class="info-text-area">' + (pointData.villageNum ? pointData.villageNum + '栋' : '') + '</div></div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '居住人口：' + '</div><div class="info-text-area">' + (pointData.villagePopul ? pointData.villagePopul + '万人': '') + '</div></div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '停车位数量：' + '</div><div class="info-text-area">' + (pointData.villageParking ? pointData.villageParking+'个' : '') + '</div></div>',
          '<div class="btn-info-map" onclick="clickDetail('+ pointData.villageId +')">点击查看详情</div>'
      ];
      let that = this;
      window.clickDetail = function (villageId) {
        that.$router.push({
          path: '/villagesDetail',
          query: {
            villageId: villageId
    ...
    villagePosition (villageId) {
      this.$axios.post('village/queryVillageDetail/'+ villageId)
      .then((resp) => {
        if(resp.data.code === 1){
  ...
  beforeRouteEnter (to, from, next) {
    next( (vm) => {
      if (from.name === 'villagesDetail') {
        vm.villagePosition(from.query.villageId)
      }
    });
  }
}
