      ...
      <div class="table-top">
        ...
        <div class="btn-table">
          ...
          <Upload action="/office/upload" 
            ...
            v-show="funcObj.func6"
            style="margin-right: 10px;">
              <Button type="primary" icon="ios-cloud-upload-outline">新增项目</Button>
          </Upload>
          <Button type="warning" class="btn-primary" v-show="funcObj.func7">
           <a href="/office/download" download="写字楼导入模板.xlsx"> <Icon type="ios-download-outline" />下载模板 </a>
          </Button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods:{
    ...
    remove (row) {
      this.$axios.post('/office/remove/'+ row.officeId)
      .then((resp) => {
        this.initTableData();
        if(resp.data.code === 1){
          this.$Notice.success({
  ...
  computed: {
    ...
    funcObj () {
      ...
      let a = list.filter(item => item.funcId == 6 ) ||[];
      let b = list.filter(item => item.funcId == 7 ) ||[];
      obj.func6 = a.length > 0;
      obj.func7 = b.length > 0;
      return obj
    }
  },
  created() {
