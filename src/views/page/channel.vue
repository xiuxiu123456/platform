<template>
  <div class="area-container">
    ...
    <div class="amap-wrapper"  id="container" >
      ...
      <div class="dt-box">
        <div class="title-dt">图例</div>
        <div class="content-dt">
          <div class="left-dt">
            <div class="pg"><img src="@/assets/img/pg1.png" alt="">公共设施综合开发</div>
            <div class="pg"><img src="@/assets/img/pg3.png" alt="">盘活存量用地</div>
            <div class="pg"><img src="@/assets/img/pg5.png" alt="">新供应用地建设</div>
            <div class="pg"><img src="@/assets/img/pg7.png" alt="">盘活存量用房</div></div><div class="right-dt">
            <div class="pg"><img src="@/assets/img/pg2.png" alt="">城际合作</div>
            <div class="pg"><img src="@/assets/img/pg4.png" alt="">货币补贴</div>
            <div class="pg"><img src="@/assets/img/pg6.png" alt="">配建</div>
          </div>
        </div>
      </div>
      <div class="dt-m">
        <div class="rjImg"  @click="showCheckItem">
            <img src="data:image/png;base64," alt="" class="img1">
            <div>建设方式</div>
            <img src="data:image/png;base64," alt="" class="img2" :class="{active:inshowItem}">
          </div>
          <div class="select-b" v-show="inshowItem">
            <CheckboxGroup  v-model="disabledGroup" @on-change="checkedProgress">
              <Checkbox :label="item.dictValue" v-for="item in projectTypeList" :key="item.dictValue">{{item.dictItem}}</Checkbox>
            </CheckboxGroup >
          </div>
      </div>
...
export default {
  ...
  methods:{
    checkedProgress (data) {
      this.checkeArray = data;
      if (!this.checkAreaId) {
        this.loadDataMap();
      } else {
    ...
    showCheckItem(){
      this.inshowItem = !this.inshowItem
    },
    projectPosition (projectId) {
      ...
      .then((resp) => {
        if(resp.data.code === 1){
          ...
          if (this.typeInfo && this.typeInfo.length > 0) {
            let str = [];
            this.typeInfo.forEach(type => {
              str.pugetDictByItemsh(getDictByItem('住房类型', type.typeName).dictItem);
            })
    ...
    initData (areaId) {
      if (areaId < 1000 || areaId === undefined){
        ...
        this.checkAreaId = undefined;
      }
      this.loadDataMap();
      this.loadDataEcharts();
    },
    loadDataEcharts () {
      ...
      this.$axios.post('/map/queryStaticByType', this.param)
      .then((resp) => {
        if(resp.data.code === 1){
    ...
    loadDataMap () {
      this.hideMarker();
      this.markerList = [];
      let loadParam = Object.assign({}, this.param);
      if (this.checkeArray.length > 0) {
        loadParam.channelArrary = this.checkeArray;
      }
      this.$axios.post('/map/queryAreaCount', loadParam)
      .then((resp) => {
        if(resp.data.code === 1){
    ...
    clickMarker (markerData) {
      if (markerData.lng && markerData.lat) {
        ...
        this.mapObj.setZoom(13);
        this.checkAreaId = markerData.areaId;
        this.queryObj.areaId = markerData.areaId;
        this.loadDataEcharts()
        //标记子级标记
        this.param.startDate = dayjs(this.queryObj.startDate).format('YYYY');
        this.param.endDate = dayjs(this.queryObj.endDate).format("YYYY");
        ...
        this.$axios.post('map/queryProjectCountByAreaId/'+markerData.areaId, loadParam)
        .then((resp) => {
          if(resp.data.code === 1){
    ...
    showProject () {
      this.pointDataArray.forEach(point => {
        this.mapObj.remove(point);
      })
      this.projectInfos.forEach(pointData => {
        console.log(pointData)
        pointData.progressStatusName = getDictByItem('进度状态', pointData.progressStatus).dictItem;
        ...
        var marker = new AMap.Marker({
          ...
          content: '<div style="display: flex;"><div class="point-pg point-pg-'+ pointData.projectType +'"></div>'+
        ...
        this.pointDataArray.push(marker);
    ...
    clickPoint (pointData) {
      this.pointFlag = true;
      let progress = pointData.progressStatusName ? pointData.progressStatusName : '';
    ...
    initEcharts () {
      ...
      this.dataList.forEach(data => {
        if (data.countNum && data.projectChannel) {
          this.num_series_data.legend_data.push(getDictByItem('建设渠道', data.projectChannel).dictItem);
          let obj = {};
          obj.name = getDictByItem('建设渠道', data.projectChannel).dictItem;
          obj.value = data.countNum;
          this.num_series_data.series_data.push(obj);
        }
        if (data.areaNum && data.projectChannel) {
          this.area_series_data.legend_data.push(getDictByItem('建设渠道', data.projectChannel).dictItem);
          let obj = {};
          obj.name = getDictByItem('建设渠道', data.projectChannel).dictItem;
          obj.value = data.areaNum;
          this.area_series_data.series_data.push(obj);
        }
        if (data.realBuildNum && data.projectChannel) {
          this.build_series_data.legend_data.push(getDictByItem('建设渠道', data.projectChannel).dictItem);
          let obj = {};
          obj.name = getDictByItem('建设渠道', data.projectChannel).dictItem;
...
.point-pg-E{
  background: url('../../assets/img/pg1.png') no-repeat;
  background-size: cover;
}
.point-pg-C{
  background: url('../../assets/img/pg3.png') no-repeat;
  background-size: cover;
}
.point-pg-A{
  background: url('../../assets/img/pg5.png') no-repeat;
  background-size: cover;
}
.point-pg-D{
  background: url('../../assets/img/pg7.png') no-repeat;
  background-size: cover;
}
.point-pg-F{
  background: url('../../assets/img/pg2.png') no-repeat;
  background-size: cover;
}
.point-pg-G{
  background: url('../../assets/img/pg4.png') no-repeat;
  background-size: cover;
}
.point-pg-B{
  background: url('../../assets/img/pg6.png') no-repeat;
  background-size: cover;
}
</style>