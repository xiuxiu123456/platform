...
export default {
  ...
  methods: {
    initData () {
      this.$axios.post('/residence/queryMapCount', this.param)
      .then((resp) => {
        if(resp.data.code === 1){
    ...
    clickMarker (markerData) {
      if ( markerData.lng && markerData.lat ) {
        ...
        this.$axios.post('residence/queryResidenceListByAreaId/'+markerData.areaId)
        .then((resp) => {
          if(resp.data.code === 1){
      ...
      this.villageInfoList.forEach(pointData => {
        var marker = new AMap.Marker({
          ...
          content: '<div style="display: flex;"><div class="point-content point-content-F"><div class="point-inset"></div></div>'+
                   '<div style="width:300px">' + pointData.residenceName + '</div></div>'
        });
        let that = this;
        marker.on('click', function(){
    ...
    clickPoint (pointData) {
      ...
      var content = [
          '<div class="title-info-map">' + pointData.residenceName + '</div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '占地面积：' + '</div><div class="info-text-area">' + (pointData.residenceArea ? pointData.residenceArea : '') + 'm<sup>2</sup></div></div>',
          ...
          '<div class="infoconten-row"><div class="infoconten-label">' + '总栋数：' + '</div><div class="info-text-area">' + (pointData.residenceTotal ? pointData.residenceTotal + '栋': '') + '</div></div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '物业类型：' + '</div><div class="info-text-area">' + (pointData.residenceType ? pointData.residenceType : '') + '</div></div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '建筑类型：' + '</div><div class="info-text-area">' + (pointData.residenceLayout ? pointData.residenceLayout : '') + '</div></div>',
          '<div class="btn-info-map" onclick="clickDetail('+ pointData.residenceId +')">点击查看详情</div>'
      ];
      let that = this;
      window.clickDetail = function (residenceId) {
        that.$router.push({
    ...
    villagePosition (residenceId) {
      this.$axios.post('residence/queryResidenceDetail/'+ residenceId)
      .then((resp) => {
        if(resp.data.code === 1){
  ...
  beforeRouteEnter (to, from, next) {
    next( (vm) => {
      if (from.name === 'residenceDetail') {
        vm.villagePosition(from.query.residenceId)
      }
    });
  }
}
