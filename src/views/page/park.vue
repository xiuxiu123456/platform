...
export default {
  ...
  methods: {
    initData () {
      this.$axios.post('/park/queryMapCount', this.param)
      .then((resp) => {
        if(resp.data.code === 1){
    ...
    clickMarker (markerData) {
      if ( markerData.lng && markerData.lat ) {
        ...
        this.$axios.post('park/queryParkListByAreaId/'+markerData.areaId)
        .then((resp) => {
          if(resp.data.code === 1){
      ...
      this.villageInfoList.forEach(pointData => {
        var marker = new AMap.Marker({
          ...
          content: '<div style="display: flex;"><div class="point-content point-content-F"><div class="point-inset"></div></div>'+
                   '<div style="width:300px">' + pointData.parkName + '</div></div>'
        });
        let that = this;
        marker.on('click', function(){
    ...
    clickPoint (pointData) {
      this.pointFlag = true;
      var content = [
          '<div class="title-info-map">' + pointData.parkName + '</div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '占地面积：' + '</div><div class="info-text-area">' + (pointData.parkArea?pointData.parkArea:'') + 'm<sup>2</sup></div></div>',
          ...
          '<div class="infoconten-row"><div class="infoconten-label">' + '总栋数：' + '</div><div class="info-text-area">' + (pointData.parkTotal?pointData.parkTotal+'栋':'') + '</div></div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '园区用途：' + '</div><div class="info-text-area">' + (pointData.parkUse?pointData.parkUse:'') + '</div></div>',
          '<div class="infoconten-row"><div class="infoconten-label">' + '是否重点园区：' + '</div><div class="info-text-area">' + (pointData.importPark?pointData.importPark:'') + '</div></div>',
          '<div class="btn-info-map" onclick="clickDetail('+ pointData.parkId +')">点击查看详情</div>'
      ];
      let that = this;
      window.clickDetail = function (parkId) {
        that.$router.push({
          path: '/parkDetail',
          query: {
            parkId: parkId
    ...
    villagePosition (parkId) {
      this.$axios.post('park/queryParkDetail/'+ parkId)
      .then((resp) => {
        if(resp.data.code === 1){
  ...
  beforeRouteEnter (to, from, next) {
    next( (vm) => {
      if (from.name === 'parkDetail') {
        vm.villagePosition(from.query.parkId)
      }
    });
  }
}
